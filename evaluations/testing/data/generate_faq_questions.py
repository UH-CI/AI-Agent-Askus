...
class QuestionGenerator:
    ...
    def generate_questions_and_answers(self, document: Dict[str, Any], num_questions: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        # Get document text based on the structure of the document
        if "metadata" in document and "document" in document.get("metadata", {}):
            doc_text = document.get("metadata", {}).get("document", "")
        else:
            doc_text = document.get("document", "") or document.get("text", "") or document.get("content", "") or document.get("extracted", "")
        
        if not doc_text:
            print("No document text found in the document structure, skipping...")
            return []
        ...
        prompt = f"""
        I'll provide you with a document containing frequently asked questions and answers. Please generate {num_questions} diverse questions that could be answered using this document. 
            ...
            for i, q_text in enumerate(cleaned_questions):
                if q_text and '?' in q_text:  # Ensure it's an actual question
                    ...
                    questions.append({
                        ...
                        "answer": answer,
                        "document_id": doc_id,
                        "document_text": doc_text,  # Include for reference
...
def main():
    ...
    parser.add_argument('--input', type=str, default='testing/policies.json',
                        help='Path to the input JSON file containing policy documents')
    
    parser.add_argument('--output', type=str, default='data/evaluation_questions.json',
